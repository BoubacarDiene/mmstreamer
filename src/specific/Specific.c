...
extern uint32_t       nbVideoConfigs;

/* -------------------------------------------------------------------------------------------- */
/*                                         PROTOTYPES                                           */
/* -------------------------------------------------------------------------------------------- */

static SPECIFIC_ERROR_E loadAllParams_f  (SPECIFIC_S *obj);
static SPECIFIC_ERROR_E unloadAllParams_f(SPECIFIC_S *obj);

static SPECIFIC_ERROR_E loadGraphicsParams_f  (SPECIFIC_S *obj);
static SPECIFIC_ERROR_E unloadGraphicsParams_f(SPECIFIC_S *obj);

static SPECIFIC_ERROR_E loadVideoParams_f  (SPECIFIC_S *obj);
static SPECIFIC_ERROR_E unloadVideoParams_f(SPECIFIC_S *obj);

static SPECIFIC_ERROR_E loadServersParams_f  (SPECIFIC_S *obj);
static SPECIFIC_ERROR_E unloadServersParams_f(SPECIFIC_S *obj);

static SPECIFIC_ERROR_E loadClientsParams_f  (SPECIFIC_S *obj);
static SPECIFIC_ERROR_E unloadClientsParams_f(SPECIFIC_S *obj);

static SPECIFIC_ERROR_E keepAppRunning_f(SPECIFIC_S *obj, SPECIFIC_KEEP_ALIVE_METHOD_E method, uint32_t timeout_s);
...
SPECIFIC_ERROR_E Specific_Init(SPECIFIC_S **obj, CONTEXT_S *ctx)
{
    ...
    if (Listeners_Init(&pData->listenersObj, ctx) != LISTENERS_ERROR_NONE) {
...
static SPECIFIC_ERROR_E loadAllParams_f(SPECIFIC_S *obj)
{
    SPECIFIC_ERROR_E ret = SPECIFIC_ERROR_NONE;
    
    if ((ret = loadGraphicsParams_f(obj)) != SPECIFIC_ERROR_NONE) {
        Loge("Failed to load graphics params");
        goto graphics_exit;
    }
    
    if ((ret = loadVideoParams_f(obj)) != SPECIFIC_ERROR_NONE) {
        Loge("Failed to load video params");
        goto video_exit;
    }
    
    if ((ret = loadServersParams_f(obj)) != SPECIFIC_ERROR_NONE) {
        Loge("Failed to load servers params");
        goto servers_exit;
    }
    
    if ((ret = loadClientsParams_f(obj)) != SPECIFIC_ERROR_NONE) {
...
static SPECIFIC_ERROR_E loadGraphicsParams_f(SPECIFIC_S *obj)
{
    ...
    *nbGfxElements = xmlGraphics->nbElements;
    ...
    if (*nbGfxElements == 0) {
        ...
        uint32_t imageId, stringId, fontId;
        int32_t colorId;
        ...
        for (index = 0; index < *nbGfxElements; index++) {
            ...
            assert((elementPData = calloc(1, sizeof(LISTENERS_ELEMENT_PDATA_S))));
	        ...
	        switch ((*gfxElements)[index]->type) {
	            ...
	            case GFX_ELEMENT_TYPE_IMAGE:
	                ...
	                imageId = xmlGraphics->elements[index].image->imageId;
	                colorId = xmlGraphics->elements[index].image->hiddenColorId;
	                
	                elementPData->ids.image.imageId       = imageId;
	                elementPData->ids.image.hiddenColorId = colorId;
	                
	                getImage_f((void*)&xmlGraphics->common, imageId, &(*gfxElements)[index]->data.image);
	                
	                if (colorId >= 0) {
	                    assert(((*gfxElements)[index]->data.image.hiddenColor = calloc(1, sizeof(GFX_COLOR_S))));
	                    getColor_f((void*)&xmlGraphics->common, colorId, (*gfxElements)[index]->data.image.hiddenColor);
	                }
	                break;
	            ...
	            case GFX_ELEMENT_TYPE_TEXT:
	                ...
	                stringId = xmlGraphics->elements[index].text->stringId;
                    fontId   = xmlGraphics->elements[index].text->fontId;
                    colorId  = xmlGraphics->elements[index].text->colorId;
	                
	                elementPData->ids.text.stringId = stringId;
	                elementPData->ids.text.fontId   = fontId;
	                elementPData->ids.text.colorId  = colorId;
                    
                    getString_f((void*)&xmlGraphics->common, stringId, graphicsInfos->currentLanguage, (*gfxElements)[index]->data.text.str);
	                getFont_f((void*)&xmlGraphics->common, fontId, (*gfxElements)[index]->data.text.ttfFont);
	                
	                (*gfxElements)[index]->data.text.ttfFontSize = xmlGraphics->elements[index].text->size;
	                getColor_f((void*)&xmlGraphics->common, (int32_t)colorId, &(*gfxElements)[index]->data.text.color);
	                break;
            ...
            elementPData->index  = index;
            strncpy(elementPData->clickHandlerName, xmlGraphics->elements[index].clickHandlerName, sizeof(elementPData->clickHandlerName));
            
            elementPData->getters.getString   = getString_f;
            elementPData->getters.getColor    = getColor_f;
            elementPData->getters.getFont     = getFont_f;
            elementPData->getters.getImage    = getImage_f;
            elementPData->getters.getLanguage = getLanguage_f;
            
            elementPData->getters.userData    = (void*)&xmlGraphics->common;
            
            (*gfxElements)[index]->pData = elementPData;
        ...
        for (index = 0; index < *nbGfxElements; index++) {
            if ((*gfxElements)[index]) {
                if ((*gfxElements)[index]->pData) {
                    ((LISTENERS_ELEMENT_PDATA_S*)(*gfxElements)[index]->pData)->getters.userData = NULL;
                    free((*gfxElements)[index]->pData);
                    (*gfxElements)[index]->pData = NULL;
                }
...
static SPECIFIC_ERROR_E loadVideoParams_f(SPECIFIC_S *obj)
{
    ...
    if (xmlVideo->configChoice >= nbVideoConfigs) {
        Loge("Chosen video config does not exist");
        goto badConfig_exit;
    }
    ...
    videoParams->caps        = gVideoConfig[xmlVideo->configChoice].caps;
    videoParams->type        = gVideoConfig[xmlVideo->configChoice].type;
    videoParams->pixelformat = gVideoConfig[xmlVideo->configChoice].pixelformat;
    videoParams->colorspace  = gVideoConfig[xmlVideo->configChoice].colorspace;
    videoParams->memory      = gVideoConfig[xmlVideo->configChoice].memory;
    videoParams->awaitMode   = gVideoConfig[xmlVideo->configChoice].awaitMode;
...
static SPECIFIC_ERROR_E keepAppRunning_f(SPECIFIC_S *obj, SPECIFIC_KEEP_ALIVE_METHOD_E method, uint32_t timeout_s)
{
    ...
    switch (method) {
        ...
        case SPECIFIC_KEEP_ALIVE_TIMER_BASED:
            sleep(timeout_s);
            break;
